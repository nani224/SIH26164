...
from engine import source_c, source_java, source_python
...
def test_k1_sensitivity_known_false_negatives_appear_as_residue() -> None:
    ...
    for filename, content, expected_signal in known_fn_fixtures:
        ...
        has_residue = any(
            expected_signal in s.producing_rule or expected_signal == s.signal_type
            for s in ledger.residue_spans
        )
        if has_residue:
...
def test_k2_specificity_benign_artifacts_below_defined_floor() -> None:
    ...
    print(
        f"\n[K2 Specificity]: benign residue mass = {total_benign_residue_mass} "
        f"over {total_benign_bytes} bytes ({residue_ratio * 100:.2f}%)"
    )
    print(f"  Defined floor: {BENIGN_RESIDUE_FLOOR_RATIO * 100:.1f}%")
    assert residue_ratio <= BENIGN_RESIDUE_FLOOR_RATIO, (
        f"K2 Specificity failed: residue ratio {residue_ratio * 100:.2f}% > "
        f"floor {BENIGN_RESIDUE_FLOOR_RATIO * 100:.1f}%"
    )
...
def test_k3_non_vacuity_disabled_rules_reappear_as_residue() -> None:
    ...
    aes_findings = [
        Span(artifact_hash=aes_hash, kind="byte", start=32, end=288, producing_rule="binary.aes_sbox")
    ]

    # Run with rule present
    aes_ledger_0 = compute_ledger(aes_binary, aes_findings, aes_suspicion, aes_hash)
    r_aes_0 = aes_ledger_0.residue_mass
    assert r_aes_0 == 0.0

    # Run with rule DISABLED (findings = [])
    aes_ledger_disabled = compute_ledger(aes_binary, [], aes_suspicion, aes_hash)
    r_aes_1 = aes_ledger_disabled.residue_mass
    assert r_aes_1 > r_aes_0, "S-box did not reappear as residue when AES rule was disabled!"
