...
_RUST_PATTERNS: list[tuple[re.Pattern[str], Family, CryptoFunction, str, int | None]] = [
    ...
    (
        re.compile(r"\b(?:ring::aead::)?CHACHA20_POLY1305\b"),
        Family.CHACHA20,
    ...
    (
        re.compile(r"\b(?:ring::digest::)?SHA1_FOR_LEGACY_USE_ONLY\b"),
        Family.SHA_1,
        CryptoFunction.DIGEST,
        "ring SHA-1 legacy digest",
        None,
    ),
    (
        re.compile(r"\b(?:ring::signature::)?ED25519\b"),
        Family.ED25519,
        CryptoFunction.SIGN,
        "ring Ed25519 signature",
        None,
    ),
    (
        re.compile(r"\b(?:ring::signature::)?RSA_PKCS1_[A-Z0-9_]+\b"),
        Family.RSA,
        CryptoFunction.SIGN,
        "ring RSA-PKCS1 signature",
        2048,
    ),
    (
        re.compile(r"\b(?:ring::agreement::)?X25519\b"),
        Family.X25519,
        CryptoFunction.KEYDERIVE,
        "ring X25519 agreement",
        256,
    ),
    (
        re.compile(r"\b(?:ring::agreement::)?ECDH_P256\b"),
        Family.ECDH,
        CryptoFunction.KEYDERIVE,
        "ring ECDH-P256 agreement",
        256,
    ),
    (
        re.compile(r"\b(?:ring::agreement::)?ECDH_P384\b"),
        Family.ECDH,
        CryptoFunction.KEYDERIVE,
        "ring ECDH-P384 agreement",
        384,
    ),
    ...
    (
        re.compile(r"\bChaCha20Poly1305(?:::new)?\b"),
        Family.CHACHA20,
        CryptoFunction.ENCRYPT,
    ...
    (
        re.compile(r"\bSigningKey::generate\b"),
        Family.ED25519,
        CryptoFunction.KEYGEN,
        "ed25519-dalek SigningKey::generate",
        256,
    ),
