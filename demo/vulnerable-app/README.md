...
A Track CC (M5) CI/CD demonstration: `legacy_auth.py` generates a
...
## Why `legacy_auth.py` isn't on `main`
...
`main` as needed: **[`demo/vulnerable-app-fixture`](../../../../tree/demo/vulnerable-app-fixture)**
(originally proven from `demo/vulnerable-rsa1024`, PR #9). To re-run the
demo:
...
Or, to see the effect without opening a real PR, just read `legacy_auth.py`
on that branch and the `demo-vulnerable-check.yml` workflow definition
side by side -- the mechanism is exactly the same one an external
consuming repository's own PRs would go through via
